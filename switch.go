...
func runSwitch(ctx context.Context, command parsedCommand) error {
...
func loadSwitchTarget(request switchRequest) (config, error) {
	if (request.Environment == "") == (request.ConfigPath == "") {
		return config{}, errors.New("switch requires exactly one environment or configuration path")
	}
	if request.Environment != "" && !environmentPattern.MatchString(request.Environment) {
		return config{}, errors.New("invalid switch environment")
	}
	if request.ConfigPath != "" && !filepath.IsAbs(request.ConfigPath) {
		return config{}, errors.New("switch configuration path must be absolute")
	}

	c, err := loadEnvironment(request.Environment, request.ConfigPath)
	if err != nil {
		return config{}, err
	}
	if c.Environment == "" {
	...
	if request.ACR {
		...
		if !namePattern.MatchString(c.RegistrySubscription) {
			return config{}, errors.New("set registry_subscription to the subscription containing ACR")
		}
	}
	return c, nil
}
