...
func TestSessionPromptLabelIsLiteral(t *testing.T) {
	c := profile.Profile{Environment: "prod$(touch /tmp/injected)%", AKS: &profile.AKS{Name: "cluster`command`"}}
	label := Label(c)
...
func TestSessionPromptPreservesShellStartup(t *testing.T) {
	...
	for _, shell := range []string{"bash", "zsh"} {
		t.Run(shell, func(t *testing.T) {
			...
			args, childEnv, err := PreparePrompt(t.TempDir(), executable, []string{"-i"}, env, profile.Profile{Environment: "staging", AKS: &profile.AKS{Name: "test-cluster"}})
			if err != nil {
				t.Fatal(err)
			}
...
`
	...
	args, env, err := PreparePrompt(t.TempDir(), shell, []string{"-i"}, []string{"PATH=" + os.Getenv("PATH"), "HOME=" + home}, profile.Profile{Environment: "staging", AKS: &profile.AKS{Name: "test-cluster"}})
	if err != nil {
		t.Fatal(err)
	}
...
source "$ZSH/oh-my-zsh.sh"`
			...
			args, childEnv, err := PreparePrompt(t.TempDir(), shell, []string{"-i"}, env, profile.Profile{Environment: "staging", AKS: &profile.AKS{Name: "test-cluster"}})
			if err != nil {
				t.Fatal(err)
			}
...
func TestDisabledPromptLeavesShellStartupUntouched(t *testing.T) {
	...
	args, env, err := PreparePrompt(directory, "/bin/zsh", []string{"-i"}, []string{"ZDOTDIR=/existing"}, profile.Profile{Prompt: &settings, Environment: "staging"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCustomPromptPlacementWithoutColour(t *testing.T) {
	...
	for _, placement := range []string{"prefix", "suffix"} {
		t.Run(placement, func(t *testing.T) {
			...
			args, env, err := PreparePrompt(t.TempDir(), shell, []string{"-i"}, []string{"HOME=" + home, "PATH=" + os.Getenv("PATH")}, profile.Profile{Prompt: &settings, Environment: "staging"})
			if err != nil {
				t.Fatal(err)
			}
