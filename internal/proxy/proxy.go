// Package proxy routes HTTPS CONNECT traffic through private tunnels or direct connections.
package proxy
...
// Router is an HTTPS-only forward proxy. ACR connections use SOCKS5 with
...
// Config defines the private routes and upstream transports used by a Router.
type Config struct {
	Registry     string
	PrivateHosts []string
	SOCKSAddress string
...
// NewRouter creates an HTTPS CONNECT router. Route and upstream proxy settings
// are copied so later configuration changes cannot retarget an active router.
func NewRouter(config Config) *Router {
	var directProxy *url.URL
	if config.DirectProxy != nil {
		copy := *config.DirectProxy
		directProxy = &copy
	}
	return &Router{
		registry:     config.Registry,
		privateHosts: append([]string(nil), config.PrivateHosts...),
...
func (p *Router) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		diagnostics.Event(r.Context(), diagnostics.EventProxyRouteFailed)
...
func (p *Router) track(connections ...net.Conn) bool {
	p.mu.Lock()
	defer p.mu.Unlock()
	if p.closed {
		return false
...
func (p *Router) untrack(connections ...net.Conn) {
	p.mu.Lock()
	defer p.mu.Unlock()
	for _, connection := range connections {
...
// ValidRemoteDNSName reports whether host is an exact lowercase DNS name that
// is safe to resolve remotely. It rejects local names and address literals.
...
func (p *Router) isTunnelHost(host string) bool {
...
func (p *Router) isRegistryHost(host string) bool {
	host = strings.ToLower(strings.TrimSuffix(host, "."))
...
func (p *Router) dialSOCKS(ctx context.Context, host, port string) (net.Conn, error) {
...
func (p *Router) dialDirect(ctx context.Context, host, port string) (net.Conn, error) {
