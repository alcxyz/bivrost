...
func openBastion(ctx context.Context, c config) (_ *bastionSession, resultErr error) {
	finish := diagnosticStep(ctx, eventBastion)
	...
	if err := waitPort(ctx, loopback(s.port), s.process); err != nil {
		return nil, err
	}
	complete = true
	return s, nil
...
func interactiveSSH(ctx context.Context, c config) (resultErr error) {
	finish := diagnosticStep(ctx, eventShell)
	defer func() { finish(resultErr) }()
