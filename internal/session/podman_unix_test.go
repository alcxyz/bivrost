...
func TestPodmanBridgeLifecycle(t *testing.T) {
	for _, ready := range []bool{true, false} {
		t.Run(fmt.Sprint(ready), func(t *testing.T) {
			...
			defer cancel()
			bridge, err := startPodmanProxyBridge(ctx, profile.Profile{ProxyPort: 28080})
			if !ready {
				if err == nil {
					bridge.stop()
...
func TestRequireProxyChecksSelectedPodmanBridge(t *testing.T) {
	...
	writeTestExecutable(t, filepath.Join(root, "podman"), podman)
	t.Setenv("PATH", root+string(os.PathListSeparator)+os.Getenv("PATH"))
	c := profile.Profile{Registry: "exampleregistry", ProxyPort: availablePort(t), SOCKSPort: availablePort(t)}
...
func TestPodmanCommandsIgnoreDockerConfiguration(t *testing.T) {
	...
	t.Setenv("PATH", root)
	c := profile.Profile{Registry: "registry", ProxyPort: 28080}
