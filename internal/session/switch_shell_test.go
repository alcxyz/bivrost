...
`
	...
	cmd.Env = shellinit.ReplaceEnvironment(os.Environ(), "BIVROST_PROMPT_ENABLED", "0")
	cmd.Env = shellinit.ReplaceEnvironment(cmd.Env, "BIVROST_EXECUTABLE", "Invoke-BivrostSwitchTest")
	cmd.Env = shellinit.ReplaceEnvironment(cmd.Env, "BIVROST_ACR_ENV_FILE", filepath.Join(t.TempDir(), "unused.json"))
	switchScript := filepath.Join(t.TempDir(), "nested switch.ps1")
	if err := os.WriteFile(switchScript, []byte("bivrost switch accepted\n"), 0600); err != nil {
		t.Fatal(err)
	}
	cmd.Env = shellinit.ReplaceEnvironment(cmd.Env, "BIVROST_TEST_SWITCH_SCRIPT", switchScript)
	output, err := cmd.CombinedOutput()
	var exitError *exec.ExitError
