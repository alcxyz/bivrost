...
func sessionLabel(c config) string {
...
func effectivePromptSettings(c config) promptSettings {
	if c.Prompt != nil {
		return *c.Prompt
	}
...
func prepareSessionPrompt(directory, shell string, args, env []string, c config) ([]string, []string, error) {
	environment := c.Environment
	if environment == "" {
		environment = "custom-profile"
	}
	...
	env = replaceEnvironment(env, "BIVROST_PROMPT_LABEL", sessionLabel(c))
	settings := effectivePromptSettings(c)
