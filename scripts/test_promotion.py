...
def event(head="dev", repo="alcxyz/bivrost", base="main"):
...
class PromotionTests(unittest.TestCase):
    def test_new_version_from_dev_is_accepted(self):
        promotion.check(event(), "0.2.0", "0.1.2", False)

    def test_unreleased_or_reused_versions_are_rejected(self):
        for current, base, tag in [("0.1.2", "0.1.2", False),
                                   ("0.1.1", "0.1.2", False),
                                   ("0.2.0", "0.1.2", True)]:
    ...
    def test_main_rejects_feature_branches_and_fork_dev(self):
        for pr in [event(head="feature"), event(repo="someone/bivrost")]:
            with self.assertRaises(ValueError):
                promotion.check(pr, "0.2.0", "0.1.2", False)

    def test_development_prs_do_not_require_a_release(self):
        promotion.check(event(head="feature", base="dev"), "0.1.2", "0.1.2", True)

    def test_versions_are_compared_numerically(self):
        promotion.check(event(), "0.10.0", "0.9.0", False)
