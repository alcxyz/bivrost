	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			services := platformServices{
				...
				startPodman: func(_ context.Context, _ config, gotDirectory string) (*podmanSession, error) {
				...
				loginPodman: func(_ context.Context, _ config, got *podmanSession) error {
			...
			for key, want := range wantEnv {
				if got := environmentValue(shellEnv, key); got != want {
