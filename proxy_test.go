...
func TestRegistryHostMatchingIsStrict(t *testing.T) {
	t.Parallel()
	router := proxyRouter{registry: "widgets"}
...
func testSOCKSRoute(t *testing.T, host string, privateHosts []string) {
	...
	router := &proxyRouter{registry: "widgets", privateHosts: privateHosts, socksAddress: socksListener.Addr().String()}
...
func TestRegistrySOCKSFailureDoesNotUseDirectProxy(t *testing.T) {
	...
	router := &proxyRouter{registry: "widgets", privateHosts: []string{"team-vault.vault.azure.net"}, socksAddress: closedAddress, directProxy: directURL}
...
func TestHTTPProxyAuthAndSanitizedFailure(t *testing.T) {
	...
	defer cancel()
	router := &proxyRouter{directProxy: proxyURL}
