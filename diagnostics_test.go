...
func testDiagnosticLog(t *testing.T) (context.Context, *diagnosticLog) {
	t.Helper()
	t.Setenv("XDG_STATE_HOME", t.TempDir())
	ctx, log, err := startDiagnostics(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
...
func readDiagnosticRecords(t *testing.T, log *diagnosticLog) []diagnosticRecord {
	t.Helper()
...
func TestDiagnosticsOnlyFixedFields(t *testing.T) {
	...
	diagnosticEvent(ctx, eventCommand)
	diagnosticEvent(ctx, diagnosticEventName(255))
	diagnosticStep(ctx, diagnosticEventName(255))(errors.New("secret-unknown-event"))
	cases := []struct {
		err     error
		outcome string
	}{
	...
	for _, c := range cases {
		end := diagnosticStep(ctx, eventAzureLogin)
		end(c.err)
...
func TestDiagnosticsPrivateDistinctFiles(t *testing.T) {
	...
	_, second, err := startDiagnostics(ctx)
	if err != nil {
		t.Fatal(err)
	}
	defer second.close()
...
func TestDiagnosticsBoundedConcurrentWrites(t *testing.T) {
	...
	for range 8 {
		...
		go func() {
			defer writers.Done()
			for range 1000 {
				diagnosticEvent(ctx, eventProxy)
			}
		}()
	}
	writers.Wait()
	...
	diagnosticEvent(ctx, eventCleanup)
...
func TestDiagnosticsRetentionDoesNotDeleteOrOverwrite(t *testing.T) {
	...
	if _, _, err := startDiagnostics(context.Background()); err == nil {
...
func TestDiagnosticsDisabledContext(t *testing.T) {
	...
	diagnosticEvent(context.Background(), eventCommand)
	diagnosticStep(context.Background(), eventAzureLogin)(errors.New("FAKE-TOKEN"))
...
func TestDiagnosticsConcurrentQuota(t *testing.T) {
	...
	logs := make(chan *diagnosticLog, diagnosticMaxFiles*3)
	for range diagnosticMaxFiles * 3 {
		starters.Add(1)
		go func() {
			defer starters.Done()
			_, log, err := startDiagnostics(context.Background())
			if err == nil {
				logs <- log
			}
		}()
...
func TestDiagnosticsRejectRelativeXDGState(t *testing.T) {
	...
	_, _, err := startDiagnostics(context.Background())
...
func TestDebugCLIRecordsConfigurationFailureWithoutItsPath(t *testing.T) {
	...
	if err := run([]string{"connect", "-c", missing}); err == nil {
	...
	if err := run([]string{"connect", "-c", missing, "-d"}); err == nil {
		t.Fatal("missing configuration accepted")
	}
