...
func newACRActivation(ctx context.Context, c config, services platformServices, directory, shell string) *acrActivation {
...
func (a *acrActivation) enable() error {
	...
	if !a.config.SkipRegistryLogin && !namePattern.MatchString(a.config.RegistrySubscription) {
		return errors.New("set registry_subscription to the subscription containing ACR")
	}
	finish := diagnosticStep(a.ctx, eventPodmanSession)
...
func activationScript(shell string, s *podmanSession) string {
	...
	powershell := strings.Contains(strings.ToLower(filepath.Base(shell)), "pwsh") || strings.HasPrefix(strings.ToLower(filepath.Base(shell)), "powershell")
	if powershell {
	...
	if s.wrapperDirectory != "" {
		b.WriteString(unixPodmanPathInit)
	}
	return b.String()
}
...
func (a *acrActivation) listen(env []string) ([]string, error) {
	...
	env = replaceEnvironment(env, "BIVROST_CONTROL_FILE", path)
	env = replaceEnvironment(env, "BIVROST_ACR_ENV_FILE", a.script)
	env = replaceEnvironment(env, "BIVROST_EXECUTABLE", exe)
	return env, nil
}
