...
func activationTestServices(t *testing.T, starts, logins *atomic.Int32) platformServices {
	...
	return platformServices{
		startPodman: func(_ context.Context, _ profile.Profile, directory string) (*podmanSession, error) {
			starts.Add(1)
...
func TestActivationFailureCleansUpAndAllowsRetry(t *testing.T) {
	for _, failAt := range []string{"registry", "login", "script"} {
		t.Run(failAt, func(t *testing.T) {
			...
			services.checkRegistry = func(context.Context, profile.Profile) error {
...
func TestActivationScriptQuotesShellMetacharacters(t *testing.T) {
	...
	for _, binary := range []string{"pwsh", "powershell"} {
		...
		t.Run(binary, func(t *testing.T) {
			...
			cmd.Env = shellinit.ReplaceEnvironment(os.Environ(), "BIVROST_TEST_DATA", path)
			out, err := cmd.CombinedOutput()
			if err != nil {
...
func TestUnifiedACRCommands(t *testing.T) {
	for _, test := range []struct {
		...
		{args: []string{"connect", "-e", "staging", "--acr"}, Kind: cli.Connect, ACR: true},
		{args: []string{"connect", "-e", "staging", "--acr", "-n"}, Kind: cli.Connect, ACR: true, NoLogin: true},
		{args: []string{"connect", "-e", "staging", "-n"}, fail: true},
		{args: []string{"acr", "enable"}, Kind: cli.ACREnable},
		{args: []string{"acr", "enable", "-e", "prod"}, fail: true},
		{args: []string{"acr", "connect", "-e", "staging", "-n"}, Kind: cli.ACRConnect, NoLogin: true},
	} {
