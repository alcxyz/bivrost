...
func doctorTestController(t *testing.T, c config, services platformServices) (*acrActivation, activationControl) {
	t.Helper()
...
func TestDoctorSessionStatusDoesNotPublishFailedActivation(t *testing.T) {
	...
	services.loginPodman = func(context.Context, config, *podmanSession) error { return errors.New("synthetic login failure") }
...
func TestDoctorWithoutTargetRequiresLiveSession(t *testing.T) {
	for _, session := range []string{"", "stale-session"} {
		t.Run(session, func(t *testing.T) {
			...
			err := runDoctor(context.Background(), parsedCommand{kind: commandDoctor}, &out)
			if err == nil {
...
func doctorTestActiveEnvironment(t *testing.T, c config, status *doctorSessionStatus) {
	t.Helper()
...
func TestDoctorExplicitTargetUsesOnlyMatchingSessionState(t *testing.T) {
	...
	for _, changed := range []bool{false, true} {
		t.Run(map[bool]string{false: "matching", true: "same registry different VM"}[changed], func(t *testing.T) {
			...
			var out bytes.Buffer
			_ = runDoctor(context.Background(), parsedCommand{kind: commandDoctor, configPath: path}, &out)
