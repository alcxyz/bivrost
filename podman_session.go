...
func startPodmanSession(ctx context.Context, c config, directory string) (result *podmanSession, resultErr error) {
	...
	session.wrapperDirectory, err = preparePodmanWrapper(directory)
	if err != nil {
		return nil, err
	}
	if machine == nil {
...
func (s *podmanSession) environment(env []string) []string {
	...
	if s.wrapperDirectory != "" {
		result = replaceEnvironment(result, "BIVROST_PODMAN_BIN", s.wrapperDirectory)
		if env != nil {
			result = replaceEnvironment(result, "PATH", s.wrapperDirectory+string(os.PathListSeparator)+environmentValue(env, "PATH"))
		}
	}
	if s.host != "" {
...
func (s *podmanSession) loginCommand(ctx context.Context, c config) (*exec.Cmd, error) {
...
func (s *podmanSession) close() {
	s.once.Do(func() {
		...
		if s.wrapperDirectory != "" {
			cleanupPodmanWrapper(s.wrapperDirectory, os.Stderr)
		}
		if s.configuration != "" {
