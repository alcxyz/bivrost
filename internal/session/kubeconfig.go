...
func prepareKubeconfig(ctx context.Context, c profile.Profile, directory string, localPort int) (_ kubeTarget, resultErr error) {
	finish := diagnostics.Step(ctx, diagnostics.EventKubeconfig)
...
func validRemoteDNSName(host string) bool {
	return proxy.ValidRemoteDNSName(host)
}
