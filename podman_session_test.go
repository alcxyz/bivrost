...
func TestPodmanSessionEnvironment(t *testing.T) {
	...
	for _, s := range []*podmanSession{{configuration: "/owned/config"}, {configuration: "/owned/config", host: "ssh://root@127.0.0.1:2222/run/bivrost-session.ABC123xyz456/api.sock", identity: "/key"}} {
		...
		if environmentValue(result, "CONTAINER_CONNECTION") != "" {
			t.Fatal("old connection retained")
		}
		if environmentValue(result, "PATH") != "/bin" || environmentValue(result, "HTTPS_PROXY") != "keep" {
			t.Fatal("unrelated environment lost")
		}
		if s.host != "" {
			if environmentValue(result, "CONTAINER_HOST") != s.host || environmentValue(result, "CONTAINER_SSHKEY") != "/key" || environmentValue(result, "CONTAINERS_CONF_OVERRIDE") != "/owned/config" {
				t.Fatal("machine environment incorrect")
			}
		} else if environmentValue(result, "CONTAINERS_CONF_OVERRIDE") != "/owned/config" || environmentValue(result, "CONTAINER_HOST") != "" {
...
func TestPodmanSessionRejectsExistingOverride(t *testing.T) {
	...
	if _, err := startPodmanSession(context.Background(), config{}, t.TempDir()); err == nil {
